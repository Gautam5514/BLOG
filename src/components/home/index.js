        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <div className="w-full lg:w-8/12">
                    <div
                        className={`${isGridView ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "space-y-6"
                            }`}
                    >
                        {filteredPosts && filteredPosts.length > 0 ? (
                            filteredPosts.map((postItem) => (
                                <article
                                    ...
                                    key={postItem._id}
                                    className={`cursor-pointer ${isGridView ? "flex flex-col" : "flex gap-6"
                                        } bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden`}
                                >
                                    <div
                                        className={`${isGridView ? "w-full h-48" : "w-1/3 h-full"
                                            } relative`}
                                    >
                                        <img
                        ...
                        <div className="shadow-md p-6 rounded-lg">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {BLOG_CATEGORIES.map((categoryItem) => (
                                    <Button
                                        className={`${currentSelectedCategory === categoryItem.key
